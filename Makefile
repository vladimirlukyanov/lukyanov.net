...
build:
	...
	docker build -t lukynaov . -f ${CURRENT_DIR}/.config/docker/watch.Dockerfile

watch:
	@echo "[✨ \033[0;32mRunning\033[0m Docker 🐳 lukynaov]"
	docker run --rm -it -v ${CURRENT_DIR}/backend:/app lukynaov npm i -f
	docker run --rm -it \
			-e ASTRO_TELEMETRY_DISABLED=1 \
			-e INDEXNOW_KEY=$(INDEXNOW_KEY) \
    		-v ${CURRENT_DIR}/backend:/app \
    		-v ${SCSSLEON_DIR}/scss:/app/src/styles/scss \
    		-p 4321:4321 lukynaov npm run dev
...
sync :
	docker run --rm -it \
    		...
    		-v ${SCSSLEON_DIR}/scss:/app/src/styles/scss \
    		lukynaov npm run build
	docker run --rm -it -v ~/.aws:/root/.aws public.ecr.aws/aws-cli/aws-cli:latest s3 rm s3://lukynaov.net --recursive
	docker run --rm -it -v ~/.aws:/root/.aws -v ${CURRENT_DIR}/frontend:/dist public.ecr.aws/aws-cli/aws-cli:latest \
			s3 cp /dist s3://lukynaov.net --recursive --exclude "_astro/*"
	docker run --rm -it -v ~/.aws:/root/.aws -v ${CURRENT_DIR}/frontend:/dist public.ecr.aws/aws-cli/aws-cli:latest \
			s3 cp /dist/_astro s3://lukynaov.net/_astro --recursive --cache-control "max-age=${CACHE_CONTROL}"
